...
class Connection {
    ...
    async auth(preemptive) {
        ...
        while (true) {
            ...
            this.setupPreemptiveReauth();

            return true;
        }
    }

    setupPreemptiveReauth() {
        ...
        let preemptiveReauthTimerDuration = (isGoogle ? API_GOOGLE_REAUTH_MINUTES : API_NEST_REAUTH_MINUTES) * 60 * 1000;
        ...
        }, preemptiveReauthTimerDuration);
        this.preemptiveReauthTimerExpiryTime = Date.now() + preemptiveReauthTimerDuration;
    }
    ...
    updateProtobufData(resolve, handler) {
        ...
        return new Promise(async (res, rej) => {
            ...
            this.debug('Note:', Math.round((this.preemptiveReauthTimerExpiryTime - Date.now()) / 1000), 's left until preemptive reauth timer expiry');
            if (this.preemptiveReauthTimerExpiryTime - Date.now() < 5 * 60 * 1000) {
                // Preemptive reauth timer has less than 5 minutes to run? Reauth now instead to avoid race condition
                // where preemptive reauth is in progress while making a new protobuf Observe call
                this.setupPreemptiveReauth();
                try {
                    this.debug('(Fast forward) Initiating pre-emptive reauthentication.');
                    await this.auth(true);
                } catch(error) {
                    this.debug('(Fast forward) Pre-emptive reauthentication failed.');
                }
            }
